...
type StructMap = { [key: string]: Struct }
...
const NOMAP: StructMap = {}
const NOARR: StructArr = []
...
Norma.parse = function(spec: any) {
  ...
  while (entry = stack.shift()) {
    ...
    let ref: any = (parent as any)[parent.type]
    let s: Struct
...
Norma.validate = function(node: any, struct: Struct, errlog: StructErr[]) {
  let root = { '': node }
  let stack: {
    node: any,
    struct: Struct,
